...
class RootView(BoxLayout):
    ...
    def on_kv_post(self, base_widget):
        # Initialize from env if present (desktop testing convenience)
        self.feed_url = os.getenv('twitcluburl', '').strip()

        # Default save location: iOS sandbox Documents or Kivy user data dir
        try:
            base_home = os.path.expanduser('~')
            # Common layout on iOS: ~/Documents
            docs = os.path.join(base_home, 'Documents')
            os.makedirs(docs, exist_ok=True)
            default_dir = docs
        except Exception:
            # Fallback to Kivy user_data_dir
            default_dir = App.get_running_app().user_data_dir if App.get_running_app() else os.path.expanduser('~')
    ...
    def pick_folder(self):
        # On iOS, stick to the app's Documents folder (Files app integration).
        try:
            d = self.save_dir or os.path.join(os.path.expanduser('~'), 'Documents')
    ...
    def start_download(self):
        ...
        save_dir = self.save_dir or os.path.join(os.path.expanduser('~'), 'Documents')
        try:
...
class ClubTwitiOSApp(App):
    title = 'Club TWiT Downloader (iOS)'
...
if __name__ == '__main__':
    ...
    # python ios.py https://example.com/path/to/rss.xml
